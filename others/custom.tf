...
# Module invocation for the IAM role management permissions with a condition.
module "gcp_org_level_binding_policies_role_management" {
  ...
  binding_condition = {
    title       = "Restrict IAM Role Management to dig- prefixed roles"
    description = "Grants permissions to manage IAM roles (create, delete, get, undelete, update) only if the role's ID starts with 'dig-'."
    expression  = "resource.name.extract('/roles/{role_id}').startsWith('dig-')"
  }
